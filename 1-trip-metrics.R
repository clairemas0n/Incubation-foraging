...
xy_df <- read.csv("C:/Users/cjmason2/OneDrive - University of Tasmania/PHD WORK/2 R files/Ch3/Master_datafiles/xy_trips_400m_edits_Nov2021_latest.csv")
...
write.csv(summary.tb, "all_trip_metrics_summary.tb_Nov2021.csv")
...
summary.tb %>% filter(year != "latewinter2018") %>% group_by(year, band) %>% tally(Yno.trips > 1) %>% count( n > 1) %>% filter(`n > 1` == TRUE) 

abc <- summary.tb %>% filter(year != "latewinter2018") %>% group_by(year, band) %>% count(no.years >=2)  %>% filter(`no.years >= 2` == TRUE)
...
# Total_dist ~ Year
m_td <- lm(total_dist ~ year, data= summary.ts_sanswinter)
summary(m_td)
anova(m_td)

# Max range ~ Year 
m_md <- lm(max_dist ~ year, data= summary.ts_sanswinter)
summary(m_md)
anova(m_md)
...
p3 <- ggplot() + 
  geom_boxplot(data = summary.tb %>% filter(year != "latewinter2018") %>% filter(year != "2019"),  
               aes(x = year, y = A)) + theme_classic(base_size=20) + ggtitle("A (trip sinuosity)") 

p4 <- ggplot() + 
  geom_boxplot(data = summary.tb %>% filter(year != "latewinter2018") %>% filter(year != "2019"),  
               aes(x = year, y = total_dist)) + theme_classic(base_size=20) + ggtitle("Total distance") 

p5 <- ggplot() + 
  geom_boxplot(data = summary.tb %>% filter(year != "latewinter2018") %>% filter(year != "2019"),  
               aes(x = year, y = max_dist)) + theme_classic(base_size=20) + ggtitle("Max range") 

ggarrange(geom_bla, p2, p3, p1, p4, p5,nrow=2, ncol=3)
